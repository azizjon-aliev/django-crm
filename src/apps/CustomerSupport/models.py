...
def get_all_subject_values(values):
	return [(key, value) for key, value_list in values.items() for key, value in value_list]
...
class CustomerRequest(TimeStampedModel, UserStampedModel):
	...
	SUBJECT_CHOICES = {
		RequestType.COMPLAINT: [
			("poor_service", _("Плохое обслуживание")),
			("billing_issues", _("Ошибки в счетах")),
			("network_problems", _("Проблемы с сетью")),
			("other", _("Другое")),
		],
		RequestType.SUGGESTION: [
			("new_feature", _("Предложение новой функции")),
			("improvement", _("Улучшение сервиса")),
			("other", _("Другое")),
		],
		RequestType.QUESTION: [
			("tariff_info", _("Информация о тарифах")),
			("service_details", _("Детали услуги")),
			("other", _("Другое")),
		],
		RequestType.TECHNICAL: [
			("connection_issues", _("Проблемы с подключением")),
			("equipment_failure", _("Неисправность оборудования")),
			("other", _("Другое")),
		],
		RequestType.BILLING: [
			("overcharge", _("Переплата")),
			("payment_failure", _("Ошибка оплаты")),
			("other", _("Другое")),
		],
		RequestType.THANKS: [
			("service_quality", _("Качество обслуживания")),
			("employee_thanks", _("Благодарность сотруднику")),
			("other", _("Другое")),
		],
	}
	...
	phone_number = models.CharField(
		...
		validators=[
			RegexValidator(
				regex=r"^\+992\d{9}$",
				message=_(
					"Номер телефона должен начинаться с +992 и содержать 9 цифр после кода страны."
				),
			),
		],
	)
	...
	subject = models.CharField(
		...
		help_text=_("Выберите тему обращения"),
		choices=get_all_subject_values(SUBJECT_CHOICES),
		blank=True,
	)
